...
func Delete_name_idHandler(cfg *config.APIConfig) func(ctx context.Context, request mcp.CallToolRequest) (*mcp.CallToolResult, error) {
	return func(ctx context.Context, request mcp.CallToolRequest) (*mcp.CallToolResult, error) {
		...
		url := fmt.Sprintf("%s/%s/%s", cfg.BaseURL, name, id)
...
func CreateDelete_name_idTool(cfg *config.APIConfig) models.Tool {
	tool := mcp.NewTool("delete_name_id",
		mcp.WithDescription(""),
	...
	return models.Tool{
		Definition: tool,
		Handler:    Delete_name_idHandler(cfg),
	}
}
